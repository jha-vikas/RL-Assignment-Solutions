...
CAR_WIDTH = 14     
CAR_HEIGHT = 8   
SENSOR_DIST = 100   # FIXED: Increased sensor range for better obstacle detection
SENSOR_ANGLE = 15   # FIXED: 15° between sensors = 90° total FOV (7 sensors spanning -45° to +45°)
SPEED = 3           # FIXED: Slower speed for better control and learning
TURN_SPEED = 8      # FIXED: Effective turning for gradual curves (degrees/step)
SHARP_TURN = 25     # FIXED: Sharp turn angle for tight corners (degrees)

# ==========================================
# REINFORCEMENT LEARNING HYPERPARAMETERS - FIXED
# ==========================================
BATCH_SIZE = 64     # FIXED: Stable batch size for learning (64 experiences per update)

GAMMA = 0.95        # FIXED: Discount factor - agent plans ~20 steps ahead
                    # Higher values = more long-term planning

LR = 0.001          # FIXED: Stable learning rate for Adam optimizer

TAU = 0.005         # FIXED: Soft target network updates (slow and stable)
                    # Low TAU = gradual target network updates = more stable learning

MAX_CONSECUTIVE_CRASHES = 5    # FIXED: Reset position after 5 consecutive crashes
                               # Quick reset to avoid wasting time on bad trajectories
...
class DrivingDQN(nn.Module):
    ...
    Enhanced with additional FC(512) layer for better feature extraction and
    improved learning capacity for complex navigation patterns.
    """
    def __init__(self, input_dim, output_dim):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(128, 256),
            nn.ReLU(),
            nn.Linear(256, 512),      # NEW: Additional layer for increased capacity
            nn.ReLU(),
            nn.Linear(512, 256),      # NEW: Bottleneck back down
            nn.ReLU(),
            nn.Linear(256, 128),
            nn.ReLU(),
...
class CarBrain:
    def __init__(self, map_image: QImage):
        ...
        self.steps = 0
        self.epsilon = 1.0   # FIXED: Start with 100% exploration, decays over time via optimize()
                             # High initial epsilon ensures thorough exploration of the map
        self.base_epsilon = 1.0  # Track the naturally decayed epsilon (before any boosts)
    ...
    def reset(self):
        ...
        # This ensures car follows learned path instead of exploring randomly
    ...
    def get_state(self):
        sensor_vals = []
        self.sensor_coords = []
        # 7 sensors dynamically spaced using SENSOR_ANGLE parameter
        # With SENSOR_ANGLE=15: angles = [-45, -30, -15, 0, 15, 30, 45] (90° total FOV)
        # Formula: (i - 3) * SENSOR_ANGLE where i = 0..6 gives 7 evenly spaced sensors
        ...
        norm_dist = min(dist / self.map_diagonal, 1.0)  # Scale-invariant normalization
    ...
    def step(self, action):
        ...
        if car_center_val < 0.4:
            ...
            self.alive = False
        elif dist < 35:  # Target detection radius
            reward = 100
            ...
            if has_next:
                ...
                # Boost epsilon when reaching the BEST target (frontier) - EVERY TIME
                # This encourages exploration beyond the known path
                # Car exploits learned path to get here, then explores to find next target
                if self.targets_reached >= self.best_target_reached:
                    if self.targets_reached > self.best_target_reached:
                        self.best_target_reached = self.targets_reached  # Update best
                    if self.epsilon < 0.3:
                        self.epsilon = 0.3  # Boost exploration at the frontier
            else:
                # ALL TARGETS COMPLETED! Mark best_target_reached as total targets
                # This prevents epsilon boost at T2 in future runs since we've been beyond it
                self.best_target_reached = len(self.targets)  # = 3 for 3 targets
                done = True
        else:
            # Reward for clear center path (sensor index 3 = 0° = straight ahead)
            # High sensor value = road ahead = good, low = obstacle = bad
            center_sensor = next_state[3]  # Center sensor at 0 degrees
            reward += center_sensor * 10   # Reward for clear path ahead
    ...
    def optimize(self):
        ...
        # Natural epsilon decay - also track base_epsilon
        if self.base_epsilon > 0.05: 
            self.base_epsilon *= 0.9999  # Decay the base value
        # If epsilon was boosted, it will be higher than base - that's intentional
        # It will be reset to base_epsilon on episode reset
        if self.epsilon > 0.05 and self.epsilon <= self.base_epsilon:
            self.epsilon = self.base_epsilon  # Keep epsilon at base level
        return loss.item()
...
class NeuralNavApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("NeuralNav: DQN Self-Driving Car - Assignment 2")
        ...
        # Logic - Load default map (map3.jpg - Paris-style city with river)
        # Alternative maps available: map1.jpg, map2.jpg, map4.jpg (night), map5.jpg
